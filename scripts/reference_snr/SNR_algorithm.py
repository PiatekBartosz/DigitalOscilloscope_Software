...
def normal_plot(data, filename=None, xmin='a', xmax='a', ymin='a', ymax='a', 
                ytick='a', xtick='a', ytitle='', xtitle='', titletext='', 
                xlin='lin', ylin='lin', color1='yellow', linewidth=4, 
                ticksize=40, titlesize=48, titlecolor='yellow', plottype='line'):
    ...
    if xlin == 'lin':
        ...
        ax.xaxis.set_minor_locator(ticker.LogLocator(base=10, subs=np.arange(1.0, 10.0), numticks=100))
        if xmin != 'a':
            ax.set_xlim(xmin, xmax)
    if ylin == 'lin':
    ...
    if xlin == 'log' or ylin == 'log':
        ax.grid(True, which='minor', color=gridcolor, linestyle='-', linewidth=0.5, alpha=0.5)
    ...
    ax.tick_params(axis='both', which='major', colors=tickcolor, 
                   labelsize=ticksize, width=2, length=10, pad=10)
    ax.tick_params(axis='both', which='minor', colors=tickcolor, 
                   width=1, length=5)
...
def remove_peak(f, spectrum, fftbinsize):
    ...
    normal_plot(data      = plotdata,
                ...
                xlin='lin', ylin='log',
                xmin=-searchrange, xmax=searchrange,
                # ymin=10e-9, ymax=40,
                linewidth=4)  
    ...
    replacementnoise = np.mean(spectrum[tone_idx+delta_f_bins:tone_idx+delta_f_bins+noisewindow])
    ...
    normal_plot(data      = plotdata,
                ...
                xtitle='Frequency (Hz)', ytitle='Level (V²/Hz)',
                filename  = basedir + '/plot2.html',
                xlin = 'lin', ylin = 'log',
                xmin = ftone-delta_f_bins*fftbinsize*10, xmax=ftone+delta_f_bins*fftbinsize*10,
                linewidth=4)
...
def remove_spurious(f, noise_spectrum):
    ...
    normal_plot(data      = plotdata,
                ...
                xtitle='Frequency (Hz)', ytitle='Level (dB)',
...
ideal_timesignal = 0 # 1 creates an ideal timesignal. 0 loads an actual measured timesignal
...
if ideal_timesignal==0:
    ...
    print('The SNR is:                  ' + str(round(20*np.log10(signal_rms/noise_rms),6)) + ' dB')
    print('The average noise is:        ' + str(round(1e6*noise_rms/np.sqrt(snrfmax-snrfmin),4)) + ' uV/rtHz' )
else:
    ...
    print('The CalculatedSNR is:        ' + str(round(20*np.log10(signal_rms/noise_rms),2)) + ' dB')
    print('The SNR error is:            ' + str(round(20*np.log10(signal_rms/noise_rms)-ideal_snr,3)) + ' dB')
